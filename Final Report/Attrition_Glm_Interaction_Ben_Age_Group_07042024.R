...
# Grouping Age
young_age <- data$Age < 30
middle_age <- data$Age >= 30 & data$Age < 55
senior_age <- data$Age >= 55
data$Age_Group <- factor(
  ifelse(young_age, "Young Age",
         ifelse(middle_age, "Middle Age",
                "Senior Age")),
  levels = c("Young Age", "Middle Age", "Senior Age")
)
...
# Demographics Training Model

my_glm_demo = glm(Attrition ~ Age_Group + 
                     MaritalStatus + NumCompaniesWorked , 
                   family = 'binomial', data = training_data)
summary(my_glm_demo)

# Predict probability for each customer
probabilities <- predict(my_glm_demo, testing_data, type = "response")
head(probabilities, 5)
probabilities
...
# Work-Life Balance Training Model

my_glm_wlb = glm(Attrition ~ DistanceFromHome + 
                    OverTime +  WorkLifeBalance , 
                  family = 'binomial', data = training_data)
summary(my_glm_wlb)

# Predict probability for each customer
probabilities <- predict(my_glm_wlb, testing_data, type = "response")
head(probabilities, 5)
probabilities
...
# Job Satisfaction and Engagement Training Model

my_glm_jse = glm(Attrition ~  
                     EnvironmentSatisfaction + JobInvolvement + JobSatisfaction, 
                   family = 'binomial', data = training_data)
summary(my_glm_jse)

# Predict probability for each customer
probabilities <- predict(my_glm_jse, testing_data, type = "response")
head(probabilities, 5)
probabilities
